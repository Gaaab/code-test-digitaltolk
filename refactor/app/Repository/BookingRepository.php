...
use App\Services\Filters\AllJobsQueryFilter;
use App\Services\Dto\SendJobNotificationDto;
use App\Services\Helpers\{ DateHelpers, UserTagsHelpers };
...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($jobs) {
            ...
            if (count($noramlJobs)) {
                $noramlJobs = collect($noramlJobs)->map(function ($item, $key) use ($user_id) {
                    return array_merge([
                        'usercheck' => Job::checkParticularJob($user_id, $item)
                    ], $item->toArray())
                })->sortBy('due')->all();
            }
        }
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        $duration = DateHelpers::convertToHoursMins($job->duration);
        $jobId = $job->id;
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        $user_tags = UserTagsHelpers::getUserTagsStringFromArray($users);
    ...
    {
        $entity = new Class implements Notifiable {
            public function __construct($user, $job, $language, $due, $duration) {
            }
        ...
        // Not recommend to put this on repository...
        return (new NotificationService(new ThirdPartNotification))->send($entity);
    }
    ...
    public function sendNotificationByAdminCancelJob($job_id)
    {
        ...
        $data = (new SendJobNotificationDto($job));

        $this->sendNotificationTranslator($job, $data->toArray(), '*');   // send Push all sutiable translators
    }
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        $allJobs->filter((new AllJobsQueryFilter(
            $cuser,
            $consumer_type
        )), $requestdata);
        ...
        if ($limit === 'all') return $allJobs->get();
